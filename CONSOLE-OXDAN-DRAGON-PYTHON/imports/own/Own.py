...
def Own():
    ...
    if imports.own.will_go_to_start.x.lower() == "cls_history": # cls_history (+)
        ...
        try:
            print("\n\033[0;33mTime Start: \033[0;37m %s" % imports.own.start_start.start_time_2)
            sleep(0.01)
            print("\033[0;33mTime Spent: \033[0;37m %s seconds, %s minutes, %s hours, %s days" % (round(time.time() - imports.own.start_start.start_time), round((time.time() - imports.own.start_start.start_time)/60), round(((time.time() - imports.own.start_start.start_time)/60)/60), round((((time.time() - imports.own.start_start.start_time)/60)/60)/24)))
            imports.own.will_go_to_start.main()
